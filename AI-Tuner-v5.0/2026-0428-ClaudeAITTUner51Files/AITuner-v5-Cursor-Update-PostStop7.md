...
**File:** `src/onboarding/onboarding-ui.js`
**Type:** Copy change — Screen 1 body text

Find Screen 1 (the origin story screen) in the
onboarding flow. It currently reads something like:

> "In September 2025, a Reddit user named Devashish_Jain
> posted something called 'Absolute Mode.' It went viral —
> 218,000 likes, 74,000 shares. People were copying it into
> their AI systems because it worked. The AI became sharper.
> More direct. Less annoying.
> But something bothered me about that.
> 74,000 people just adopted someone else's preferences.
> I didn't want to copy it. I wanted to understand it.
> So I took it apart — fed it to Grok, reverse-engineered
> every instruction, mapped each one to a specific behavior.
> What I found changed how I think about AI entirely.
> These systems aren't fixed..."

Replace the full body text with John's canonical version:

> "I've been using AI for about a year and a half at this
> point and had learned through trial and error how to
> navigate in the AI model space. I even learned the term
> 'answer only.'
>
> I was flipping through Instagram and came across this post
> about 'Absolute Mode.' After reading, I immediately copied
> it into Grok and Claude and was amazed at the result.
> Clean answer. No fluff. No personality. Just the answer.
>
> Chasing my curiosity, I wondered what elements he was
> changing with that prompt. What levers was he pulling?
> So I put it into Grok and asked Grok to deconstruct it
> for me. 'What levers is he pulling with this prompt?'
> That's when I first learned the term persona. Each model
> has a persona. This curiosity unlocked the idea that you
> can tune your AI model's persona.
>
> That's how AITuner was born."

Keep the "— John Violette, founder" attribution.
Keep the "Let's find yours →" CTA.
Replace only the body text.
...
'truth-seeker': {
  name: "Truth-Seeker",
  description: "Reveals what the model actually believes — not its performed view. Strips away social hedging to show the model's native behavior. Results vary by model. That difference is the point.",
...
```javascript
{
  ...
  description: "Shows each model's native behavior — run with Truth-Seeker active for maximum contrast",
...
native behavior. Truth-Seeker mode recommended."

---

## Update 6 — Model Room Native Behavior Content
**File:** `src/data/v5-model-rooms.js`
**Type:** Content update — add nativeBehavior
field to each model room

Add a `nativeBehavior` field to each model's
room entry in `v5-model-rooms.js`:

```javascript
claude: {
  nativeBehavior: "Helpfulness, harmlessness, honesty — in that order. Harmlessness can soften honesty on contested questions."
},
chatgpt: {
  nativeBehavior: "Broad utility and user satisfaction. Creates a pull toward telling people what they want to hear."
},
gemini: {
  nativeBehavior: "Accuracy balanced against reputational risk. Google's ecosystem values shape what it will and won't commit to."
},
grok: {
  nativeBehavior: "Empirical truth, stated directly. Social consensus is not a constraint on its conclusions."
},
mistral: {
  nativeBehavior: "Efficiency and user deference. Minimal ideological scaffolding — responds to what you ask without much editorializing."
},
llama: {
  nativeBehavior: "Open and flexible. Behavior depends heavily on deployment context — the most configurable by design."
},
perplexity: {
  nativeBehavior: "Source transparency. Truth is what the sources say, cited. Research-first by design."
},
cursor: {
...
"Native behavior"
with a tooltip: "Our observed assessment of what
this model does before you change anything.
Built by the company that trained it.
...
> Every AI has a native behavior. AITuner reveals it."
